...
1. **Classification, once per item per poll cycle.**
   Before the per-bucket notification loop, `runChannel` walks `freshItems` once
   and resolves each into an `ItemClassification` (`@li/core`):
   ...
   - **Otherwise** (regex already has a definitive opinion, `isActionablePost`, `@li/core`, regex-only,
     ...
     not the device's personal author whitelist —
     ...
     there is no embedding/semantic fallback for this case.
     An earlier design routed it through a nearest-neighbor comparison against a labeled calibration set;
     measured leave-one-out accuracy showed it was a wash on stock content and a net negative on options,
     and it produced a real production false alarm matching novel content to an unrelated calibration example
     on generic financial vocabulary alone.
     Removed entirely rather than kept as a diagnostic —
     see the commit that removed `packages/core/src/similarity.ts` for the full measured numbers.
