...
Instead, once per ET calendar day per channel (`shouldSweepAccess`, gated via `lastValidationEnqueueDate` in the `run:<channel>` STATE blob), `runChannel`'s bucket-building loop — already paging through that channel's `TOKENS` entries to build notify buckets — collects any registration whose own `lastValidated` (`TokenMeta`, stamped at registration time and after each successful revalidation) isn't from today, and enqueues one `ValidationQueueMessage` per registration onto `VALIDATION_QUEUE`. This collection step makes no `fetch()` calls, so it costs nothing against the subrequest cap regardless of device count.
...
A confirmed-valid token gets `lastValidated` stamped with the *original* `expiration` preserved (KV `put()` requires re-specifying TTL or it's dropped) — revalidation never resets the TTL clock, so a device that's stopped re-registering still expires on schedule.
