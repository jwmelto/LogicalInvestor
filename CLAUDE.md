...
Two independent staleness checks gate this, deliberately using different representations. `lastValidationEnqueueDate` (an ET calendar-date string in the `run:<channel>` STATE blob, checked via `shouldSweepAccess`) gates the cheap scan itself — once per ET day per channel — and firing an extra time near a calendar-day boundary costs nothing, since the scan makes no `fetch()` calls, only a KV list. `lastValidated` (`TokenMeta`, an epoch-millisecond timestamp, checked via `needsRevalidation` against a rolling 24h window) gates whether one specific registration is actually stale enough to enqueue, since enqueueing costs a real subrequest in the consumer — a calendar-date string would be wrong here, since it would treat two revalidations minutes apart as both "needing" a recheck whenever they straddle midnight.
