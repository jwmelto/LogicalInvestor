...
1. **Classification, once per item per poll cycle.** Before the per-bucket notification loop, `runChannel` walks `freshItems` once and resolves each into an `ItemClassification` (`@li/core`): `{ members: boolean, actionable: boolean }`. `members` is checked first — a plain `feedKey === membersArea` comparison — and short-circuits `actionable` entirely (never computed, regex or embeddings) whenever it's true, since Members Area bypasses every tier regardless of actionable-ness. When `members` is false, `actionable` is resolved against that item's own feed's `ActionableStrategy` (`actionableStrategyFor(feedKey)`, `@li/core`) — a `{ posPatterns, calibration }` pair, one per forum's discourse rather than a feed-identity check at each call site: Members Forum and Stock Insights share `STOCK_PICK_STRATEGY` (tranche-pricing vocabulary, differing only in the star-gate Stock Insights requires), Options Insights has its own `OPTIONS_STRATEGY` (strike/put-or-call/expiry contract vocabulary, calibrated from a separate training corpus — real options tickers aren't obfuscated in it, unlike the stock corpus, since they're too time-sensitive to matter if seen). Given a strategy, `actionable` is resolved by whichever of these applies:
   - **The regex check against that strategy's `posPatterns` is undecided** (`classifySignal` returns `fail-no-signal`/`fail-too-short`, i.e. neither `NEG_PATTERNS` nor the strategy's own `posPatterns` match). All such items across the whole poll cycle are batched into **one** `env.AI.run()` call (the `[ai]` binding in `wrangler.toml`), embedded via `@cf/baai/bge-large-en-v1.5`, and each resolved through `classifyActionableHybrid` (`packages/core/src/similarity.ts`) against its own strategy's calibration vectors. Batching one call per poll cycle, not one per item, is deliberate — the same content would otherwise get re-embedded once per notification bucket. A failed/errored AI call falls back to `actionable: false` for just that cycle's candidates, never blocks the run.
   - **Otherwise** (regex already has a definitive opinion) — `isActionablePost` (`@li/core`, regex-only, also used as the AI-failure fallback): author is in the Worker's own `ACTIONABLE_AUTHORS` list (`env.ACTIONABLE_AUTHORS`, default "Sean Hyman") — not the device's personal author whitelist — the topic title starts with `*` for Stock/Options Insights, and content passes both the actionable-signal negative and positive pattern checks (`matchNegativePattern`/`matchPositivePattern`, against that item's own feed's `posPatterns`).
