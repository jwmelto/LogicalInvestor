...
**Post-registration access revalidation**: a subscription can lapse after registration, so every registration's `feedToken` is periodically reconfirmed and access-revoked ones are pruned before they get another channel's worth of content pushed to them — same signal `findAndStorePollToken` uses for stale poll tokens.
This is fully decoupled from `runChannel`'s notify path (issue #86): `runChannel` no longer calls `feedTokenHasAccess` inline at all, so the notify-tick's subrequest budget is unaffected by registered-device count.
Two independent staleness checks gate this, both epoch-millisecond timestamps checked against a rolling 24h window via the same `needsRevalidation` predicate: `lastValidationEnqueueDate` (`run:<channel>` STATE blob) gates the cheap scan itself, and `lastValidated` (`TokenMeta`) gates whether one specific registration is actually stale enough to enqueue. A calendar-date string was tried first for `lastValidationEnqueueDate` and rejected: it treats two events minutes apart as both "due" whenever they straddle a calendar-day boundary, which is a real correctness risk for `lastValidated` specifically (enqueueing costs a real subrequest in the consumer) and merely redundant complexity for `lastValidationEnqueueDate` (the scan itself makes no `fetch()` calls, so imprecision there is free either way) — an epoch timestamp carries strictly more information than a date string, since it can always be converted to a calendar day when that reasoning is genuinely needed (`DailyStats`/`advanceDaily`'s daily-bucket counters, which do need calendar-day semantics), while the reverse conversion is lossy.
`runChannel`'s bucket-building loop — already paging through that channel's `TOKENS` entries to build notify buckets — collects any registration whose own `lastValidated` (stamped at registration time and after each successful revalidation) is due, and enqueues one `ValidationQueueMessage` per registration onto `VALIDATION_QUEUE`.
`VALIDATION_QUEUE`'s consumer (`drainValidationQueue`, dispatched from the same `queue()` export that also drains `WEBPUSH_QUEUE`, distinguished via `batch.queue`) does the actual `feedTokenHasAccess` fetch, one per message, bounded by `wrangler.toml`'s consumer `max_batch_size` the same way webpush sends are — this is what makes the design scale to any registered-device count, not just up to the 50-subrequest ceiling a single invocation could ever process directly.
A confirmed-valid token gets `lastValidated` re-stamped with the *original* `expiration` preserved (KV `put()` requires re-specifying TTL or it's dropped) — revalidation never resets the TTL clock, so a device that's stopped re-registering still expires on schedule.
A confirmed-revoked token (`access === false`) is deleted outright. A transient check failure (`access === null`, e.g. a network blip) leaves the registration untouched, `lastValidated` un-stamped, so it's picked up again by the next day's scan — same tolerance `runChannel`'s other transient-failure paths already use.
Real tradeoff, unchanged from the design this replaced: a revoked subscriber can keep receiving pushes for up to a day before being pruned, instead of immediately.
