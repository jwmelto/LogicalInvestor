...
describe('actionableStrategyFor', () => {
  it('has an empty pattern set for Members Area, which bypasses the actionable tier entirely', () => {
    expect(actionableStrategyFor(FeedKeys.membersArea)).toEqual({ posPatterns: [], needsIntentConfirmation: new Set(), suppressibleLabels: new Set() });
  });
