...
// New topic ID subscriptions: { [topicId]: boolean }. Versioned key, mirroring
// topicService's discovered_topics_v3: topicId's identity scheme is slug-based (see
// topicService.generateTopicId), so a prior title-based scheme's entries must live under a
// different key rather than being silently misread as slug-based ones. 'topic_id_subscriptions'
// (no suffix) held title-based entries and is now retired — see storageService's OBSOLETE_KEYS,
// which sweeps it on every launch. Reusing that exact name here previously caused the live store
// to be swept right along with the truly-obsolete one; keep the two apart on any future bump too.
